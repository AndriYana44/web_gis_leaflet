...
class M_lahan extends CI_Model
{
	...
	{
		$this->db->distinct();
		$this->db->select('isi_lahan');
		$this->db->from('tbl_lahan');
		return $this->db->get()->result();
	}
	public function jenis()
	{
		$query = $this->db->query('SELECT *, COUNT(*) AS total FROM `tbl_lahan` GROUP BY isi_lahan ORDER BY id_lahan');
		return $query->result();
	}
	public function total_lahan()
	{
		$query = $this->db->count_all_results('tbl_lahan');
		return $query;
	}
}
